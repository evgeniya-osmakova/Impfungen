...
describe('Workspace export actions', () => {
  ...
  it('triggers PDF export and shows alert on export error', async () => {
    ...
    expect(await findByRole('alert')).toHaveTextContent(
      'Не удалось выгрузить файл. Попробуйте ещё раз.',
    );
  ...
  it('imports valid CSV and shows report summary', async () => {
    const user = userEvent.setup();
    vi.mocked(parseVaccinationCompletedImportCsv).mockReturnValue(
      createParseResult({
        rows: [createParsedImportRow()],
        totalDataRows: 1,
      }),
    );
    vi.mocked(importCompletedVaccinations).mockResolvedValue(
      createImportReport({
        importedRows: 1,
        totalDataRows: 1,
      }),
