...
import { vaccinationRepositoryLocal } from './vaccinationRepositoryLocal';
import { sortRecordsByNextDueDate } from '../../utils/vaccinationSelectors';

import { createVaccinationStoreForTests, type VaccinationStoreHook } from './index';

describe('vaccinationStore', () => {
  let store: VaccinationStoreHook;
  ...
  beforeEach(() => {
    ...
    const { store: storeInstance } = createVaccinationStoreForTests({
      repository: vaccinationRepositoryLocal,
    });
  ...
  it('confirms country and persists data', () => {
    store.getState().confirmCountry('RU');
  ...
  it('adds completed dose to history and removes matching planned dose', () => {
    ...
    const errorCode = store.getState().submitCompletedDose({
  ...
  it('recalculates repeat next due from factual completed date', () => {
    ...
    store.getState().submitCompletedDose({
      batchNumber: null,
    ...
    const [record] = sortRecordsByNextDueDate(store.getState().records);
  ...
  it('returns validation error from submitRecord and does not update records', () => {
    const errorCode = store.getState().submitRecord({
      batchNumber: null,
      completedAt: '2024-01-10',
      completedDoseKind: 'nextDose',
      diseaseId: 'measles',
  ...
  it('clears editing state after deleting record', () => {
    ...
    store.getState().startEditRecord('measles');

    store.getState().removeRecord('measles');
